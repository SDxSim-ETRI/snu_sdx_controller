...
class RobotController(Node):
    def __init__(self):
        ...
        self.declare_parameter('robot_name', 'fr3')  # Default to 'fr3'
        robot_name = self.get_parameter('robot_name').get_parameter_value().string_value
        assert robot_name in ['fr3'], f"{robot_name} is not included in fr3!"
        ...
        self.mobile_kv = 200
        self.q = np.zeros(self.m.nq)
        self.qdot = np.zeros(self.m.nv)
        self.tau = np.zeros(self.m.nu)
